...
public class SimpleTutorial : MonoBehaviour
{
    ...
    public void ScrollToItemWithIndex10()
    {
        if (adapterParams.data == null || adapterParams.data.Count > 10)
            _Adapter.SmoothScrollTo(10, 1f, null);
    }

    // Updating the models list and notify the adapter that it changed; 
    // it'll call GetItemHeight() for each item and InitOrUpdateItemViewHolder for the visible ones
    ...
    public void test()
    {
        _Adapter.SmoothScrollTo(122345, 1f);
    }
 
    public void test2()
    {
        _Adapter.NotifyScrollViewSizeChanged();
    }
    ...
    public class MyScrollRectAdapter : ScrollRectItemsAdapter8<MyParams, MyItemViewsHolder>, ExpandCollapseOnClick.ISizeChangesHandler
    {
        ...
        protected override void InitOrUpdateItemViewHolder(MyItemViewsHolder newOrRecycled)
        {
            if (newOrRecycled.root == null) // container empty => instantiate the prefab in it
            {
                ...
                newOrRecycled.root.GetComponent<ExpandCollapseOnClick>().sizeChangesHandler = this;
            }
