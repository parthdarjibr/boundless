...
namespace frame8.ScrollRectItemsAdapter.Util.GridView
{
        ...
        where TCellVH : CellViewsHolder, new()
    {
        ...
        /// <summary> Creates the Group viewholder which instantiates the group prefab using the provided params in <see cref="GridAdapter{TParams, TCellVH}.Init(TParams)"/></summary>
        ...
        /// <summary>The only important callback for inheritors. It provides cell's views holder which has just become visible and whose views should be updated from its corresponding data model. <see cref="CellViewsHolder.itemIndex"/> can be used to know what data model is associated with. </summary>
