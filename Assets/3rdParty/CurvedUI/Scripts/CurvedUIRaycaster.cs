...
*/
		...
		/*
		#region METHODS FROM OVRINPUTMODULE
        ...
        public virtual bool RaycastToCyllinderCanvas(Ray ray3D, out Vector2 o_canvasPos, bool OutputInCanvasSpace = false, int myLayerMask = -1)
        {
            ...
            if (Physics.Raycast(ray3D, out hit, float.PositiveInfinity, myLayerMask))
            {
				...
				// SHOW THE RETICLE
				CUIGazePointer.instance.SetPosition(hit.point);
				CUIGazePointer.instance.RequestShow ();
        ...
        protected virtual void ProcessMove(PointerEventData pointerEvent)
        {
            ...
            ProcessScroll(targetGO);
            HandlePointerExitAndEnter(pointerEvent, targetGO);
        }

        protected void ProcessScroll(GameObject go)
        {
            if(go != null)
            {
                // Check for scrollmanager in parent
                ScrollManager sm = go.GetComponentInParent<ScrollManager>();
                if(sm != null)
                {
                    sm.shouldScroll = true;
                }
