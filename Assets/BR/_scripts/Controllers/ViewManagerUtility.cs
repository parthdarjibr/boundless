	...
	public class ViewManagerUtility : MonoBehaviour
	{
		...
		public void SetupErrorView(ErrorDetail ed) {
			...
			if (v != null) {
				...
				/*
			// Setup the ui for the error panel
			foreach (ErrorDetail.ResponseType response in ed.responseType) {
				if (response == ErrorDetail.ResponseType.ACCEPT) {
					Transform btnAccept = UIHelper.FindDeepChild (errorObject.transform, "BtnAccept");
					btnAccept.gameObject.SetActive (true);
					btnAccept.GetComponent<Button> ().onClick.AddListener (RemoveCanvasFromCurrentView);
				} else if (response == ErrorDetail.ResponseType.CANCEL) {
					Transform btnReject = UIHelper.FindDeepChild (errorObject.transform, "BtnReject");
					btnReject.gameObject.SetActive (true);

					btnReject.GetComponent<Button> ().onClick.AddListener (RemoveCanvasFromCurrentView);
				} else if (response == ErrorDetail.ResponseType.DECLINE) {
					Transform btnCancel = UIHelper.FindDeepChild (errorObject.transform, "BtnCancel");
					btnCancel.gameObject.SetActive (true);

					btnCancel.GetComponent<Button> ().onClick.AddListener (RemoveCanvasFromCurrentView);
				}
			}*/
		...
		/// </summary>
		public void RemoveCanvasFromCurrentView() {
		...
		public void BackButtonPressed()  {
			...
			CanvasObject c = canvasStack.Peek () as CanvasObject;
			// debugText.text = canvasStack.Count.ToString();
			...
			case ViewObject.ObjectType.VIDEO:
				if (c != null && c.canvasType == CanvasObject.CanvasType.ERROR) {
					RemoveCanvasFromCurrentView ();
				} else {
					...
					if (_videoPlayer != null) {
						AnalyticsManager.Instance ().SendVideoAnalytics (_videoPlayer.currentVideo.name, 
							...
							_videoPlayer.currentVideo.uniqueId);
					}
