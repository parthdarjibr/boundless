...
public class OVRRaycaster : GraphicRaycaster, IPointerEnterHandler
{
	...
	public Vector2 GetScreenPositionCUI(RaycastResult raycastResult) {
		Vector3 rayPos = raycastResult.worldPosition;
		CurvedUISettings s = FindObjectOfType<CurvedUISettings> ();

		//direction from the cyllinder center to the hit point
		Vector3 localHitPoint = canvas.transform.worldToLocalMatrix.MultiplyPoint3x4(rayPos);

		// Get cylinder mid point
		Vector3 cyllinderMidPoint = new Vector3(0, 0, -s.GetCyllinderRadiusInCanvasSpace());
		...
		float angle = -UIHelper.AngleSigned(directionFromCyllinderCenter.ModifyY(0), s.Angle < 0 ? Vector3.back : Vector3.forward, Vector3.up);
		...
		pointOnCanvas.x = angle.Remap(-s.Angle / 2.0f, s.Angle / 2.0f, -canvasSize.x / 2.0f, canvasSize.x / 2.0f);
		pointOnCanvas.y = localHitPoint.y;

		Vector3 o_canvasPos = canvas.worldCamera.WorldToScreenPoint(canvas.transform.localToWorldMatrix.MultiplyPoint3x4(pointOnCanvas));
		return o_canvasPos;
	}
